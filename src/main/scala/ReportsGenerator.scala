...
object ReportsGenerator {
  ...
  def main(args: Array[String]) {
    ...
    val europeanCountries = spark.read.option("header", true).option("inferSchema", true).csv(resourcesPathPrefix + "countries_europe.csv")

    val countriesPopulation = spark.read.option("header", true).option("inferSchema", true).csv(resourcesPathPrefix + "countries_population.csv")

    val pathPrefix =  "file:///"+ covidDataPath.get + "/csse_covid_19_data/csse_covid_19_daily_reports/"
    val data = spark.read.option("header", true).option("inferSchema", true).option("mode","FAILFAST").option("timestampFormat", "MM/dd/yy HH:mm").csv(pathPrefix + "03-2[3-9]*", pathPrefix + "03-3[0-9]*", pathPrefix + "0[4-9]-*").cache()


    val dataWithDay = data.select("Country_Region", "Confirmed", "Deaths", "Recovered", "Active", "Last_Update").withColumn("Day", to_date(col("Last_Update"))).cache()


    val winCountry = org.apache.spark.sql.expressions.Window.partitionBy("Country_Region").orderBy("Day")
    val preparedData = dataWithDay.groupBy("Country_Region", "Day").sum("Deaths", "Confirmed", "Recovered").withColumn("Daily_Confirmed_Rate", col("sum(Confirmed)") - lag("sum(Confirmed)", 1).over(winCountry)).withColumn("Daily_Confirmed_%", round((col("Daily_Confirmed_Rate") - lag("Daily_Confirmed_Rate", 1).over(winCountry)) / lag("Daily_Confirmed_Rate", 1).over(winCountry) * 100, 2)).withColumn("Daily_Deaths_Rate", col("sum(Deaths)") - lag("sum(Deaths)", 1).over(winCountry)).withColumn("Daily_Deaths_%", round((col("Daily_Deaths_Rate") - lag("Daily_Deaths_Rate", 1).over(winCountry)) / lag("Daily_Deaths_Rate", 1).over(winCountry) * 100, 2)).withColumn("Daily_Recovered_Rate", col("sum(Recovered)") - lag("sum(Recovered)", 1).over(winCountry)).cache().withColumn("Daily_Recovered_%", round((col("Daily_Recovered_Rate") - lag("Daily_Recovered_Rate", 1).over(winCountry)) / lag("Daily_Recovered_Rate", 1).over(winCountry) * 100, 2))


    // append population
    val dataWithPopulation = preparedData.join(countriesPopulation, preparedData("Country_Region") === countriesPopulation("Country")).withColumn("Infected_%", round(col("sum(Confirmed)") / col("Population") * 100, 2)).drop("Country").cache()

    (europeanCountries.select("country").collect().toSeq.map(r => r.get(0)) :+ "US").foreach(country =>reflect.io.File("out/" + country + ".html").writeAll(toHTMLString(dataWithPopulation.filter("Country_Region = '" + country + "'").sort($"Day".desc))))

    val formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd")
    val yesterday = java.time.LocalDate.now.minusDays(1).format(formatter)

    reflect.io.File("out/Global_Daily_Confirmed_Rate.html").writeAll(toHTMLString(dataWithPopulation.filter("Day = '" + yesterday + "'").sort($"Daily_Confirmed_Rate".desc)))
